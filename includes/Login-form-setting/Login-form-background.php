...
jQuery(function() {
	...
	jQuery( "#button-size-slider4" ).slider("value",<?php if($login_form_width != ""){echo $login_form_width;}else{echo "350";}?>);
		...
		<div class="panel panel-primary panel-default content-panel">
			<div class="panel-body">
				<table class="form-table">
					<tr>
						<th scope="row" ><?php _e('Login from Floating','WEBLIZAR_ACL')?></th>
						<td></td>
					</tr>
					<tr class="radio-span" style="border-bottom:none;">
						<td>
							<span>
								<input type="radio" name="login_form_float" value="left" id="login_form_float1" <?php if($login_form_float=="left")echo "checked"; ?> />&nbsp;<?php _e('Left','WEBLIZAR_ACL')?><br>
							</span>
							<span>
								<input type="radio" name="login_form_float" value="center" id="login_form_float2" <?php if($login_form_float=="center")echo "checked"; ?> />&nbsp;<?php _e('Center','WEBLIZAR_ACL')?><br>
							</span>
							<span>	
								<input type="radio" name="login_form_float" value="right" id="login_form_float3" <?php if($login_form_float=="right")echo "checked"; ?> />&nbsp;<?php _e('Right','WEBLIZAR_ACL')?><br>
							</span>
						</td>
					</tr>
				</table>
...
<script> 
	function Custom_login_login(Action, id){
		if(Action == "loginbgSave") {
			...
			if (document.getElementById('login_form_float1').checked) {
				var login_form_float = document.getElementById('login_form_float1').value;
			}
			else if (document.getElementById('login_form_float2').checked){
				var login_form_float = document.getElementById('login_form_float2').value;
			}
			else{
				var login_form_float = document.getElementById('login_form_float3').value;
			}
			...
			var PostData = "Action=" + Action + "&Login_bg_value=" + Login_bg_value + "&login_form_float=" + login_form_float + "&login_background_color=" + login_background_color + "&login_bg_color_overlay=" + login_bg_color_overlay + "&login_bg_image=" + login_bg_image + "&login_form_opacity=" + login_form_opacity  + "&login_form_width=" + login_form_width + "&login_form_radius=" + login_form_radius + "&login_border_style=" + login_border_style + "&login_border_thikness=" + login_border_thikness + "&login_border_color=" + login_border_color + "&login_bg_repeat=" + login_bg_repeat + "&login_bg_position=" + login_bg_position + "&login_enable_shadow=" + login_enable_shadow + "&login_shadow_color=" + login_shadow_color;
			jQuery.ajax({
				dataType : 'html',
				type: 'POST',
				url : location.href,
		...
		if(Action == "loginbgReset") {
			...
			jQuery.ajax({
				...
				success: function(data) {
					...
					//Login From floating
					jQuery(document).ready( function() {
						jQuery('input[name=login_form_float]').val(['center']);
					});
					//login Background Effect
